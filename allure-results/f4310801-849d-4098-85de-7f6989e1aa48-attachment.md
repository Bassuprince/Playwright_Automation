...
Call log:
  - waiting for getByRole('textbox', { name: 'billing address line 2' })
    - locator resolved to <input value="" data-slot="input" id="addr-billing-line2" placeholder="Address line 2" aria-label="billing address line 2" class="h-9 w-full min-w-0 rounded-md border border-input bg-transparent px-3 py-1 text-base shadow-xs transition-[color,box-shadow] outline-none selection:bg-primary selection:text-primary-foreground file:inline-flex file:h-7 file:border-0 file:bg-transparent file:text-sm file:font-medium file:text-foreground placeholder:text-muted-foreground disabled:pointer-events-none disabled…/>
    - fill("Balmata Road")
