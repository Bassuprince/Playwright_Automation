...
*/
    ...
    const uniqueId = String(Date.now() % 1000).padStart(3, '0');
    const uniqueEmail = `test${uniqueId}@example.com`;
    const uniqueFirstName = `${user.firstName}${uniqueId}`;
    ...
    await page.locator('[formcontrolname="firstName"]').fill(uniqueFirstName);
    await page.locator('[type="lastName"]').fill(user.lastName)
    await page.locator('[type="email"]').fill(uniqueEmail)
  ...
  //Loginto application
 // await page.goto('https://rahulshettyacademy.com/client/#/auth/login')
  await page.locator('[placeholder="email@example.com"]').fill(uniqueEmail)
