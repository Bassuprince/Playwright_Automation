...
test.describe('Contacts Application - End to End Flow', () => {
    ...
    test('View the company Details',async ({browser}) =>{
        const context = await browser.newContext()
        const page = await context.newPage()
        ...
        await page.getByRole('button', { name: 'Add company' }).click()
        await page.locator('[id="name"]').fill('IDS Software3');
        await page.locator('[id="industry"]').fill('IT');
        await page.locator('[id="company_code"]').fill('IDS124556676');
        await page.locator('[id="founded_year"]').fill('2010');
        await page.locator('[id="annual_revenue"]').fill('1000000');
        await page.locator('[id="website"]').fill('https://ultimateqa.com');
 ...
 //==============VIEW==============
        const companyRow = page.locator('tbody tr').filter({ hasText: 'IDS Software3' });
        await expect(companyRow).toHaveCount(1);
        await companyRow.click();
        await expect(page.getByText('IDS124556676')).toBeVisible();
        await expect(page.getByRole('heading', { name: 'IDS Software3' })).toBeVisible();
        await expect(page.getByRole('paragraph').filter({ hasText: 'IT' })).toBeVisible();
        await expect(page.getByRole('link', { name: 'IDS@example.com' })).toBeVisible();
        await page.getByRole('link', { name: 'Back to companies' }).click();
        await expect(page.getByRole('link', { name: '+' })).toBeVisible();
    })

    test.skip('Delet the company Details',async ({browser}) => {
        const context = await browser.newContext()
        const page = await context.newPage()
        await localStorage(page,email)
        //====================Add company========================
        await page.locator('a .lucide.lucide-building2').click();
        await expect(page.getByText('No companies yet')).toBeVisible();
        await page.getByRole('button', { name: 'Add company' }).click()
    ...
    test.skip('Import for contact details',async ({browser})=>{
        const context = await browser.newContext()
        const page = await context.newPage()
         await page.goto('https://contacts-app-qa.idsinternal.com/login');
        //=======================Login to application========================
        await page.getByRole('textbox', { name: 'Work email' }).fill('testuser_1780894544651@example.com');
        await page.getByRole('button', { name: 'Continue' }).click();
        await page.getByRole('textbox', { name: 'Password' }).fill('Password@123');
        await page.getByRole('button', { name: 'Sign In' }).click();
        await page.waitForLoadState('networkidle');
        ...
        await expect(page.getByRole('heading', { name: 'Export contacts' })).toBeVisible();
